...
class ResearchOrchestrator:
    ...
    Streams Server-Sent Events (SSE) so the frontend can show live progress.
    """
    ...
    async def run(self, request: ResearchRequest) -> AsyncGenerator[str, None]:
        # ── Stage 1: Search ──────────────────────────────────────────
        yield self._sse("searching", "Searching academic databases…", 5)
        papers = await self.search_agent.search(request)
        if not papers:
        ...
        async def bounded_analyze(paper, idx):
            async with semaphore:
                return await self.analysis_agent.analyze(paper, request.topic)

        analysis_tasks = [bounded_analyze(p, i) for i, p in enumerate(papers)]
        summaries = []
        ...
        themes = await self.synthesis_agent.cluster_themes(papers, summaries, request.topic)
        ...
        review = await self.synthesis_agent.write_review(papers, summaries, themes, request.topic)
        ...
        implications = await self.synthesis_agent.generate_implications(review, summaries, request.topic)
